...
const generateMockAdvertisers = () => {
  ...
  const industries = ["Technology", "E-commerce", "Finance", "Healthcare", "Education", "Entertainment"]
  const advertisers = []
...
export default function AdminAdvertisersPage() {
  ...
  const [filteredAdvertisers, setFilteredAdvertisers] = useState(allAdvertisers)
  ...
  useEffect(() => {
    ...
    // Status filter
    if (statusFilter !== "all") {
      filtered = filtered.filter((advertiser) => advertiser.status === statusFilter)
    }

    // Industry filter
    if (industryFilter !== "all") {
      filtered = filtered.filter((advertiser) => advertiser.industry === industryFilter)
    }
  ...
  const handleExportAdvertisers = () => {
    toast({
      ...
      description: "Advertiser data export has been initiated. You'll receive an email when ready.",
    })
  }
  ...
  const PaginationComponent = () => (
    <div className="flex items-center justify-between px-2 py-4">
      <div className="text-sm text-muted-foreground">
        Showing {startIndex + 1} to {Math.min(endIndex, filteredAdvertisers.length)} of {filteredAdvertisers.length}{" "}
        entries
      </div>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">Manage advertiser applications and accounts</p>
        </div>
                      ...
                      <TableCell>
                        <div className="flex items-center space-x-3">
                          <Avatar className="h-8 w-8">
                            <AvatarImage src={advertiser.avatar || "/placeholder.svg"} alt={advertiser.companyName} />
                            <AvatarFallback>{advertiser.companyName.charAt(0)}</AvatarFallback>
                          </Avatar>
                          <div>
                            <div className="font-medium">{advertiser.companyName}</div>
                            <div className="text-sm text-muted-foreground">{advertiser.contactName}</div>
                          </div>
                        </div>
                      </TableCell>
                      <TableCell>{getIndustryBadge(advertiser.industry)}</TableCell>
                      <TableCell>{getStatusBadge(advertiser.status)}</TableCell>
                      <TableCell>
                        <div className="flex items-center space-x-1">
                          <DollarSign className="h-4 w-4 text-green-600" />
                          <span className="font-medium">{formatCurrency(advertiser.totalSpent)}</span>
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="text-sm">
                          <div className="font-medium">{advertiser.activeCampaigns} active</div>
                          <div className="text-muted-foreground">{advertiser.completedCampaigns} completed</div>
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="flex items-center space-x-1">
                          <TrendingUp className="h-4 w-4 text-yellow-500" />
                          <span className="font-medium">{advertiser.averageRating}</span>
                        </div>
                      </TableCell>
                      <TableCell>{formatDate(advertiser.joinedAt)}</TableCell>
                      <TableCell>
                        <div className="flex space-x-2">
                          <Button size="sm" variant="ghost" onClick={() => handleViewAdvertiser(advertiser)}>
                          ...
                          {advertiser.status === "pending" && (
                            <>
                              <Button
                                size="sm"
                                variant="ghost"
                                onClick={() => handleAdvertiserAction(advertiser.id, "approve")}
                                disabled={isLoading}
                              ...
                              <Button
                                size="sm"
                                variant="ghost"
                                onClick={() => handleAdvertiserAction(advertiser.id, "reject")}
                                disabled={isLoading}
                          ...
                          {advertiser.status === "active" && (
                            <Button
                              size="sm"
                              variant="ghost"
                              onClick={() => handleAdvertiserAction(advertiser.id, "suspend")}
                              disabled={isLoading}
                          ...
                          {advertiser.status === "suspended" && (
                            <Button
                              size="sm"
                              variant="ghost"
                              onClick={() => handleAdvertiserAction(advertiser.id, "activate")}
                              disabled={isLoading}
                              className="text-green-600 hover:text-green-700"
                            >
                              <CheckCircle className="h-4 w-4" />
        ...
        <Dialog open={advertiserDetailsDialog} onOpenChange={setAdvertiserDetailsDialog}>
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              <DialogTitle>Advertiser Details</DialogTitle>
              <DialogDescription>Detailed information about the advertiser</DialogDescription>
            </DialogHeader>
            {selectedAdvertiser && (
              <div className="space-y-6">
                <div className="flex items-center space-x-4">
                  <Avatar className="h-16 w-16">
                    ...
                    />
                    <AvatarFallback className="text-lg">{selectedAdvertiser.companyName.charAt(0)}</AvatarFallback>
                  </Avatar>
                  <div>
                    <h3 className="text-xl font-semibold">{selectedAdvertiser.companyName}</h3>
                    <p className="text-muted-foreground">{selectedAdvertiser.email}</p>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="space-y-1 text-sm">
                      <p>
                        <strong>Contact Person:</strong> {selectedAdvertiser.contactName}
                      </p>
                      <p>
                  ...
                  <div>
                    ...
                    <div className="space-y-1 text-sm">
                      <p>
                        <strong>Joined:</strong> {formatDate(selectedAdvertiser.joinedAt)}
                      </p>
                      <p>
                        <strong>Last Active:</strong> {formatDate(selectedAdvertiser.lastActive)}
                      </p>
                      <p>
                        <strong>Rating:</strong> {selectedAdvertiser.averageRating}/5.0
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{selectedAdvertiser.description}</p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="space-y-1 text-sm">
                      <p>
                        <strong>Total Spent:</strong> {formatCurrency(selectedAdvertiser.totalSpent)}
                      </p>
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="space-y-1 text-sm">
                      <p>
                        <strong>Active Campaigns:</strong> {selectedAdvertiser.activeCampaigns}
                      </p>
                      <p>
                        <strong>Completed Campaigns:</strong> {selectedAdvertiser.completedCampaigns}
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setAdvertiserDetailsDialog(false)}>
                Close
              </Button>
